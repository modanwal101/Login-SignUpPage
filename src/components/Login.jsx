...
const Login = () => {
  ...
  return (
<div className='w-[430px] bg-white p-8 rounded-2xl shadow'>

      <div className='flex justify-center mb-4'>
       <h2 className='text-3xl font-semibold text-center'>
           {isLoginMode ? "Login" : "Sign Up"}</h2>
  ...
  <button
    ...
    className={`w-1/2 text-lg font-medium transition-all z-10 ${isLoginMode ? 'text-white text-black' : 'bg-transparent text-gray-500'}`}
  >
    Login
  </button>
  <button
  ...
  <div className={`absolute top-0 h-full w-1/2 rounded-full bg-gradient-to-r from-blue-700 via-cyan-600 to-cyan-200 transition-all duration-300 ${
...
/>
        ...
        {isLoginMode && (
          <div className='text-right'><p className='text-cyan-600 hover:underline'
>Forgot Password?</p></div>
        )}

        <button type="submit" className='w-full p-3 bg-gradient-to-r from-blue-700 via-cyan-600 to-cyan-200 text-white rounded-full text-lg font-medium hover:opacity-90 transition-all'>{isLoginMode ? "Login" : "Sign Up"}</button>
